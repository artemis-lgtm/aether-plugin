"""
CRT v7 -- the bloom must come from WITHIN the tube.
...
def apply_crt_v6(portrait_path, output_path, size=(400, 400)):
    ...
    dissolve = np.clip(lum_norm * 3.5, 0, 1)  # full dissolve above ~28% brightness
    for c in range(3):
    ...
    bg.save(output_path)
    print(f"Saved CRT v7: {output_path} ({w}x{h})")

apply_crt_v6("/tmp/aether-art/austin-portrait-square.jpg",
             "/tmp/aether-art/crt_portrait_v7_large.png", size=(400, 400))
apply_crt_v6("/tmp/aether-art/austin-portrait-square.jpg",
             "/tmp/aether-art/crt_portrait_v7.png", size=(175, 175))
