"""
CRT v9 -- dimmed to make face readable.
Same as v8 but: lower initial brightness, reduced bloom, softer sheen.
"""
import numpy as np
...
def apply_crt_v6(portrait_path, output_path, size=(400, 400)):
    ...
    img = ImageEnhance.Color(img).enhance(1.5)  # more vivid phosphors
    img = ImageEnhance.Contrast(img).enhance(1.7)
    img = ImageEnhance.Brightness(img).enhance(1.0)  # between v8(1.2) and v9(0.9)
    ...
    dissolve = np.clip(lum_norm * 4.0, 0, 1) ** 0.7  # smooth wide falloff, full by ~25%
    for c in range(3):
        grille[:, :, c] = grille[:, :, c] * (1 - dissolve) + 1.0 * dissolve
    adaptive_scan = scanline * (1 - dissolve) + dissolve  # COMPLETE dissolve in highlights
    ...
    b3 = np.array(bloom_src.filter(ImageFilter.GaussianBlur(radius=22))).astype(np.float32)
    b4 = np.array(bloom_src.filter(ImageFilter.GaussianBlur(radius=45))).astype(np.float32)
    ...
    # Combine: slightly more bloom than v9 but less than v8
    bloom = b1 * 0.42 + b2 * 0.34 + b3 * 0.22 + b4 * 0.13
    ...
    for y in range(h):
        for x in range(w):
            ...
            sheen[y, x] = math.exp(-arc**2 / 0.015) * 12  # softer sheen
            # Broad secondary
            arc2 = -nx2 * 0.3 - ny2 * 0.7 + 0.3
            sheen[y, x] += math.exp(-arc2**2 / 0.04) * 4
    for c in range(3):
        final[:, :, c] += sheen
    ...
    bg.save(output_path)
    print(f"Saved CRT v8: {output_path} ({w}x{h})")

apply_crt_v6("/tmp/aether-art/austin-portrait-square.jpg",
             "/tmp/aether-art/crt_portrait_v10_large.png", size=(400, 400))
apply_crt_v6("/tmp/aether-art/austin-portrait-square.jpg",
             "/tmp/aether-art/crt_portrait_v10.png", size=(175, 175))
