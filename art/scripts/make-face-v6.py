...
try:
    ff_bold = "/System/Library/Fonts/Supplemental/Arial Bold.ttf"
    if not os.path.exists(ff_bold):
        ff_bold = "/System/Library/Fonts/Supplemental/Futura.ttc"
    if not os.path.exists(ff_bold):
        ff_bold = ff
except:
    ff_bold = ff
...
label_font = ImageFont.truetype(ff_bold, 14)  # slightly bigger, bold
...
def sharpie_bold(img, text, x, y, font, color=(10, 8, 5), center=False):
    """Extra bold sharpie -- double stroke for sharpness on tape."""
    d = ImageDraw.Draw(img)
    if center:
        bb = d.textbbox((0,0), text, font=font)
        x = x - (bb[2] - bb[0]) // 2
    # Thicker strokes for sharpness
    for dx in range(-1, 2):
        for dy in range(-1, 2):
...
KNOB_R_UV = 30  # slightly smaller clearance since knobs are smaller now
...
for name, kx, ky in knob_data:
    ...
    sharpie_bold(img, name, kx, ly + 4, label_font, center=True)

# SYNC
strip = make_duct_tape_strip(60, 22)
paste_strip(img, strip, 697, 443, angle=random.uniform(-1, 1))
sharpie_bold(img, "SYNC", 727, 447, label_font, center=True)
...
out = "/tmp/aether-art/dalle_face_v6.png"
