...
OUTPUT_PATH = '/tmp/aether-art/dalle_render_SHIP5.png'
...
# ============================================================
# GUITAR CABLES (1/4" jacks plugged in)
# ============================================================
# Cable material: black rubber
mat_cable = bpy.data.materials.new('CableRubber')
mat_cable.use_nodes = True
cnt = mat_cable.node_tree
cnt.nodes.clear()
c_out = cnt.nodes.new('ShaderNodeOutputMaterial')
c_bsdf = cnt.nodes.new('ShaderNodeBsdfPrincipled')
c_bsdf.inputs['Base Color'].default_value = (0.02, 0.02, 0.02, 1.0)
c_bsdf.inputs['Roughness'].default_value = 0.82
c_bsdf.inputs['Subsurface Weight'].default_value = 0.03
# Cable texture
c_noise = cnt.nodes.new('ShaderNodeTexNoise')
c_noise.inputs['Scale'].default_value = 300.0
c_noise.inputs['Detail'].default_value = 8.0
c_bump = cnt.nodes.new('ShaderNodeBump')
c_bump.inputs['Strength'].default_value = 0.08
c_bump.inputs['Distance'].default_value = 0.001
cnt.links.new(c_noise.outputs['Fac'], c_bump.inputs['Height'])
cnt.links.new(c_bump.outputs['Normal'], c_bsdf.inputs['Normal'])
cnt.links.new(c_bsdf.outputs['BSDF'], c_out.inputs['Surface'])

# Plug connector material: chrome/nickel
mat_plug = mat_chrome  # reuse chrome material

cable_radius = 0.025

for side, jx_sign, cable_dir in [('Input', -1, -1), ('Output', 1, 1)]:
    jx = jx_sign * (pedal_w / 2)
    jy = 0
    jz = pedal_h * 0.4
    
    # 1/4" plug barrel (cylinder going into jack)
    plug_len = 0.12
    plug_x = jx + cable_dir * (0.03 + plug_len / 2)
    bpy.ops.mesh.primitive_cylinder_add(vertices=24, radius=0.028, depth=plug_len,
        location=(plug_x, jy, jz))
    plug = bpy.context.active_object
    plug.name = f'Plug_{side}'
    plug.rotation_euler = (0, math.radians(90), 0)
    bpy.ops.object.shade_smooth()
    plug.data.materials.append(mat_plug)
    
    # Plug grip/strain relief (wider section)
    grip_x = jx + cable_dir * (0.03 + plug_len + 0.04)
    bpy.ops.mesh.primitive_cylinder_add(vertices=24, radius=0.038, depth=0.08,
        location=(grip_x, jy, jz))
    grip = bpy.context.active_object
    grip.name = f'PlugGrip_{side}'
    grip.rotation_euler = (0, math.radians(90), 0)
    bpy.ops.object.shade_smooth()
    grip.data.materials.append(mat_cable)
    
    # Cable coming out of plug, curving down to desk surface
    # Use a series of small cylinders to form a curve
    import numpy as np
    num_segs = 20
    cable_start_x = jx + cable_dir * (0.03 + plug_len + 0.08)
    
    for i in range(num_segs):
        t = i / num_segs
        # Cable curves from plug height down to desk, then extends outward
        cx = cable_start_x + cable_dir * t * 2.0
        cz = jz * (1.0 - t) + 0.015  # descend to near desk level
        # Slight curve in Y
        cy = jy + math.sin(t * math.pi * 0.5) * 0.3 * (1 if side == 'Input' else -1)
        
        # Each segment
        seg_len = 0.12
        bpy.ops.mesh.primitive_cylinder_add(vertices=12, radius=cable_radius, depth=seg_len,
            location=(cx, cy, cz))
        seg = bpy.context.active_object
        seg.name = f'Cable_{side}_{i}'
        
        # Orient toward next point
        if i < num_segs - 1:
            t2 = (i + 1) / num_segs
            nx = cable_start_x + cable_dir * t2 * 2.0
            nz = jz * (1.0 - t2) + 0.015
            ny = jy + math.sin(t2 * math.pi * 0.5) * 0.3 * (1 if side == 'Input' else -1)
            
            dir_vec = Vector((nx - cx, ny - cy, nz - cz)).normalized()
            up = Vector((0, 0, 1))
            seg.rotation_euler = dir_vec.to_track_quat('Z', 'Y').to_euler()
        
        bpy.ops.object.shade_smooth()
        seg.data.materials.append(mat_cable)
