#!/usr/bin/env python3
"""
V13 face: Opaque title bg, bigger/bolder labels.
"""
...
label_font = ImageFont.truetype(ff, 16)  # bigger, bolder
...
def sharpie_bold(img, text, x, y, font, color=(10, 8, 5), center=False, thin=False):
    ...
    if center:
        ...
        x = x - (bb[2] - bb[0]) // 2
    if thin:
        # Single stroke, slight bleed only
        for dx, dy in [(-1,0),(1,0),(0,-1)]:
            d.text((x+dx, y+dy), text, fill=(*color, 30), font=font)
        d.text((x, y), text, fill=(*color, 220), font=font)
    else:
        for dx in range(-1, 2):
            for dy in range(-1, 2):
...
# TITLE -- EVEN BIGGER
# ============================================================
# ---- BIG TAPE STRIP TITLE (no neon, red sharpie) ----
...
title_y = 8
paste_strip(img, title_strip, title_x, title_y, angle=random.uniform(-0.8, 0.8))
# Red sharpie text -- thick, hand-written look
title_font_big = ImageFont.truetype(ff, 48)
sharpie_bold(img, title_text, W // 2, title_y + 14, title_font_big, 
             color=(200, 25, 25), center=True, thin=False)
...
knob_data = [
    ...
    ('Shimmer', 420, 240), ('Space', 490, 240), ('Rate', 560, 240),
...
for name, kx, ky in knob_data:
    lw = max(65, len(name) * 12 + 20)
    lh = 26
    strip = make_duct_tape_strip(lw, lh)
    ly = ky + KNOB_R_UV + 6
    lx = kx - lw // 2
    paste_strip(img, strip, lx, ly, angle=random.uniform(-1.8, 1.8))
    sharpie_bold(img, name, kx, ly + 4, label_font, center=True, thin=False)

# SYNC
strip = make_duct_tape_strip(55, 22)
paste_strip(img, strip, 600, 443, angle=random.uniform(-1, 1))
sharpie_bold(img, "SYNC", 627, 447, label_font, center=True, thin=False)

# UPBEAT toggle label
strip_up = make_duct_tape_strip(70, 22)
paste_strip(img, strip_up, 635, 443, angle=random.uniform(-1, 1))
sharpie_bold(img, "UPBEAT", 670, 447, label_font, center=True, thin=False)
...
out = "/tmp/aether-art/dalle_face_v13.png"
