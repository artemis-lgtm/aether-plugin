...
OUTPUT_PATH = '/tmp/aether-art/dalle_render_v9.png'
FACE_ART = '/tmp/aether-art/dalle_face_v6.png'
...
def make_knob(name, u, v, mat, radius=0.06, height=0.055):
    """Pretty Princess Sparkle style: smooth dome, single slot, no ridges."""
    x, y, z = uv_to_pedal(u, v)
    
    # Knob body -- smooth cylinder, slightly tapered
    ...
    # Domed top -- hemisphere (half sphere squished)
    bpy.ops.mesh.primitive_uv_sphere_add(segments=32, ring_count=16,
        radius=radius * 0.92, location=(x, y, z + height))
    ...
    # Indicator SLOT (vertical notch cut into the top)
    bpy.ops.mesh.primitive_cube_add(size=1,
        location=(x, y + radius * 0.45, z + height + radius * 0.22))
    slot = bpy.context.active_object
    slot.name = name + '_slot'
    slot.scale = (0.003, radius * 0.55, 0.015)
    bpy.ops.object.transform_apply(scale=True)
...
cam_dist = 6.0  # whole pedal visible
cam_x = 0
...
camera.data.lens = 70  # whole pedal, minimal surround
# Depth of field
